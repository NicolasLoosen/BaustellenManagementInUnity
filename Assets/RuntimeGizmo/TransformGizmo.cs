...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
        ...
        //References the axes in the input editor in unity
        public string PrimaryClickAxis = InputAxis.PRIMARY_CLICK;
        public string SecondaryClickAxis = InputAxis.SECONDARY_CLICK;
        public string SetMoveTypeAxis = InputAxis.MOVEMENT_GIZMO;
		public string SetRotateTypeAxis = InputAxis.ROTATION_GIZMO;
		public string SetScaleTypeAxis = InputAxis.SCALING_GIZMO;
		//public KeyCode SetRectToolType = KeyCode.T;
		public string SetAllTransformTypeAxis = InputAxis.EMPTY;
		public string SetSpaceToggleAxis = InputAxis.EMPTY;
		public string SetPivotModeToggleAxis = InputAxis.EMPTY;
		public string SetCenterTypeToggleAxis = InputAxis.EMPTY;
		public string SetScaleTypeToggleAxis = InputAxis.EMPTY;
        public string AddSelectionAxis = InputAxis.MULTISELECT;
		public string RemoveSelectionAxis = InputAxis.DESELECT;
		public string ActionKeyAxis = InputAxis.DESELECT; //Its set to shift instead of control so that while in the editor we dont accidentally undo editor changes =/
        public string UndoRedoActionAxis = InputAxis.UNDO_REDO;
		...
		public bool autoSelectSiblings = true;
		...
		void LateUpdate()
		{
			...
			{
				float roundingAngle = 11.25f;

				for (int i = 0; i < targetRootsOrdered.Count; i++)
				{
					Transform target = targetRootsOrdered[i];
					float x = target.rotation.eulerAngles.x;
					float y = target.rotation.eulerAngles.y;
					float z = target.rotation.eulerAngles.z;
					float closestAngleX = (float)Math.Floor((x % 360 + roundingAngle / 2f) / roundingAngle)* roundingAngle;
					float closestAngleY = (float)Math.Floor((y % 360 + roundingAngle / 2f) / roundingAngle)* roundingAngle;
					float closestAngleZ = (float)Math.Floor((z % 360 + roundingAngle / 2f) / roundingAngle)* roundingAngle;
					
					target.rotation = Quaternion.Euler(closestAngleX, closestAngleY, closestAngleZ);
		...
		void HandleUndoRedo()
		{
			...
			if(Input.GetButton(ActionKeyAxis))
			{
				if(Input.GetButtonDown(UndoRedoActionAxis) && Input.GetAxisRaw(UndoRedoActionAxis) > 0)
				{
					UndoRedoManager.Undo();
				}
				else if(Input.GetButtonDown(UndoRedoActionAxis) && Input.GetAxisRaw(UndoRedoActionAxis) < 0)
				{
		...
		{
			if(Input.GetButton(ActionKeyAxis)) return;

			if(Input.GetButtonDown(SetMoveTypeAxis)) transformType = TransformType.Move;
			else if(Input.GetButtonDown(SetRotateTypeAxis)) transformType = TransformType.Rotate;
			else if(Input.GetButtonDown(SetScaleTypeAxis)) transformType = TransformType.Scale;
			//else if(Input.GetKeyDown(SetRectToolType)) type = TransformType.RectTool;
			else if(Input.GetButtonDown(SetAllTransformTypeAxis)) transformType = TransformType.All;

			if(!isTransforming) translatingType = transformType;

			if(Input.GetButtonDown(SetPivotModeToggleAxis))
			{
			...
			if(Input.GetButtonDown(SetCenterTypeToggleAxis))
			{
        ...
        void UpdateSelectedInPhaseManager() {
            for (int i = 0; i < targetRootsOrdered.Count; i++)
            {
				constructionSiteManager.constructionPhasesManager.UpdateObject(targetRootsOrdered[i].gameObject, constructionSiteManager.CurrentModificationOption);
            }
        }
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0) && !UnityEngine.EventSystems.EventSystem.current.IsPointerOverGameObject()) //JSC: !Unity...  added to prevent that Gizmo tries to grab object when right click menu is overlayed
			{
				bool isAdding = Input.GetButton(AddSelectionAxis);
				bool isRemoving = Input.GetButton(RemoveSelectionAxis);
				...
				if(Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
				{
					...
					if(hitInfo.transform.parent != null && autoSelectSiblings == true && (hitInfo.transform.parent.GetComponent<ObjectData>() != null)) {
						target = hitInfo.transform.parent;
					} else {
						target = hitInfo.transform;
					}
					ObjectData mb = target.gameObject.GetComponent<ObjectData>();

					if (mb != null && mb.AllowMove == true)
					{
		...
		public void ClearTargets(bool addCommand = true)
		{
            ...
            foreach (var target in targetRoots) { 
                target.Key.GetComponent<Rigidbody>().isKinematic = true;
            }
